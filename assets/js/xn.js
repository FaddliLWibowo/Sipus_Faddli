...
var xn = window.xn = (function() {
    var xn = {
        ...
        helper: {
            ...
            stylize: function(selector) {
                ...
                $(selector).find('.breadcrumb li:not(:last)').append('<span class="divider">&bull;</span>');
            ...
            alert: function(msg, severity) {
                ...
                $('.' + severity).each(function() {
                    ...
                    $(this).prepend('<button type="button" class="close" data-dismiss="alert">x</button>').css('left', ( $(document).innerWidth() - $(this).width() ) / 2 - 30 );
                    $(this).fadeIn('slow');
                });
            },
    ...
    $(function() {
        $(".login-pane").parent().parent().parent().addClass("body-login");
...
function timer() {
    ...
    $('.xinix-date').html(t.format('longDate'));
}
